# 🛡️ CDCT Testing (Consumer-Driven Contract Testing)

Este repositório é uma implementação prática de **Testes de Contrato Orientados ao Consumidor** utilizando o framework **Pact**.

O objetivo é demonstrar como garantir a integração segura entre microsserviços (Frontend e Backend) sem a necessidade de testes de integração lentos e frágeis.
...
1.  **O Consumidor (Frontend Fake)**: Define um "contrato" (expectativa) de como a API deve responder. Ao rodar este teste, um arquivo JSON é gerado.
2.  **O Contrato (Pact File)**: Um arquivo JSON que serve como "lei" entre as partes.
3.  **O Provedor (API Real)**: Lê o arquivo de contrato e verifica se sua implementação atual atende às expectativas do consumidor.
...
Este comando simula o Frontend fazendo requisições. Se passar, ele gera o arquivo de contrato na pasta `pacts/`.
...
Para rodar o ciclo completo (Geração + Verificação):
...
cdct-testing/ ├── src/ │ └── api/ # Código da API (Express) ├── tests/ │ └── contract/ │ ├── consumer/ # Testes que geram o contrato (.spec.ts) │ └── provider/ # Testes que validam a API (.spec.ts) ├── pacts/ # Contratos JSON gerados automaticamente ├── .github/workflows/ # Pipeline de CI/CD └── package.json

## 🤖 CI/CD (GitHub Actions)

Este projeto conta com um workflow configurado em `.github/workflows/contract-tests.yml`.
Toda vez que um `push` ou `pull_request` é feito na branch `main`, o GitHub Actions:

1.  Instala as dependências.
2.  Roda os testes do consumidor (gerando o contrato).
3.  Roda os testes do provedor (validando o contrato).
4.  Garante que nenhuma mudança quebre a integração.
